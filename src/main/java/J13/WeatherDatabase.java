...
public class WeatherDatabase {
    ...
    public static void main(String[] args) {
        try (Connection connection = DriverManager.getConnection(DB_URL, USER, PASSWORD)) {
            ...
            System.out.println("\nДни со снегом и температурой ниже -5 в регионе 'Москва':");
...
//            }
    ...
    public static List<String> findWeatherInRegion(Connection connection, String regionName) throws SQLException {
        String query = """
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                results.add(String.format("Дата: %s, Температура: %.1f, Осадки: %s",
    ...
    public static List<String> findSnowyDaysWithLowTemperature(Connection connection, String regionName, double temperatureThreshold) throws SQLException {
        String query = """
                    SELECT w.date
                    FROM weather w
                    JOIN region r ON w.region_id = r.id
    ...
    public static List<String> findWeatherForLanguage(Connection connection, String language) throws SQLException {
        String query = """
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            ...
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                results.add(String.format("Регион: %s, Дата: %s, Температура: %.1f, Осадки: %s",
